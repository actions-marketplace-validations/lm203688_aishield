...
- **[零安装的第一次接触：把「扫一个 MCP server」的成本降到一次粘贴](blog-online-scan-launch-2026-09-19.html)** — `2026-09-19`  
  能力不缺、用户不涨，卡在第一次接触的摩擦。`https://aishield.tools/scan` 把摩擦降到一次粘贴（零安装零注册，直接复用已测试的 `/api/v1/scan/client-config` 与 `/api/v1/export/sarif`，同一引擎、同一脱敏）；边界如实披露——在线页是服务端分析，内容会发送到 aishield.tools，要完全本机走 `npx aishield-mcp-server` 或新增的 `collector --watch`。伴随修正：删掉页面上有歧义的「代码不出机」，并以测试钉死不得回退。  
- **[Promotion Kit — 2026-09-19（零安装在线扫描 + Collector）](promotion-kit-2026-09-19-online-scan.html)** — `2026-09-19`  
  X 线程 / HN / Reddit（中英）/ V2EX·掘金 各平台文案草稿 + 待发布清单 + 不变量自查（含新增的「数据流向披露」红线）。
