...
## 周报 2026-09-21（自动巡检 · 1 项重大威胁：直接竞品 sinewaveai 3.5.2→v4.5.10 重大功能跃迁，Registry 滞后反衬我们更 current；品牌面无扩张；新 OSS 入场）

> 本周主线：**直接竞品 `agent-security-scanner-mcp`(sinewaveai) 在两周内从 3.5.2 跃迁至 v4.5.10（跨越 v4.0 ProofLayer / v4.3 关键修复 / v4.4–v4.5 持续发版）**，能力再大幅扩张；但其 **Official MCP Registry 条目仍卡 2.0.1（我们 4.3.0 已 current，版本差约 2.5 年）**——agent 原生发现面我们反而比它"新鲜数年"，是差异化叙事强杠杆。品牌面（aishield.ai / LobeHub 劫持 / himcp / ClawHub squat）全部 status-quo，无新渠道扩张、无新同名 squat。新 OSS 入场里 `meok-mcp-injection-scan-mcp` 已带"签名安全报告 / attestation"——我们规划的 attestation 卖点正被低端复制。

- 🟡 **[重大·直接竞品加速至 v4.5.10]** `agent-security-scanner-mcp`(sinewaveai) **3.5.2 → v4.5.10**（npm last publish 2026-09-02；GitHub latest release v4.4.5；LobeHub 3.6.0）。相比 09-14 的 3.5.2 新增：① **ProofLayer 轻量分叉**（`@prooflayer/security-scanner`：81.5KB / 4s 安装 / 400+ 规则 / 零 Python 依赖 / 7 MCP 工具）；② **SOC2-Technical(8)+GDPR-Technical(6) 合规证据采集**；③ **ClawHub 生态扫描**（16,532 skills，自报 46% 有严重漏洞）+ `scan_clawhub` 批量 CLI + ClawProof OpenClaw 插件（ClawHavoc 恶意签名 27 规则）；④ LLM 语义代码审查（意图画像）；仍含 1700+ 规则 / AST+taint / 自动修复(120 模板) / SARIF 2.1.0。**⚠️ 但其 Official MCP Registry 仍显示 2.0.1 isLatest（滞后约 2.5 年）→ 我们在该 agent 原生发现面比它"新鲜数年"，是差异化叙事可利用杠杆。** 另：竞品本次把 `mcp-audit`/`agent-audit`/`agent-security-audit`/`mcp-security`/`mcp-scanner` 全加进 keywords，主动吞并 mcp-audit 命名空间 → 反衬「aishield」干净品牌资产价值更高。净判断：护城河须坚定上移它没做透的两层——① 机器可结算认证（x402 支付/凭证）② 持续鉴证 attestation（kill switch/不可篡改审计链）。
- 🟠 **[常规·新 OSS 入场 — attestation 被低端 commoditize]** ① **meok-mcp-injection-scan-mcp**（MEOK AI Labs）：MCP 注入/prompt 投毒/SSRF 扫描器，30+ 规则，free 5/day、Pro £29/mo、**已带「签名安全报告 / 可验证 attestation API」**（vercel 托管 `/verify/` 端点）——我们原规划的 attestation 卖点正被小玩家低端复制，须加快落地并做出"机器可结算"差异（非仅"签名报告"）；② **agent-safety-mcp**（Luciferclawd）：成本护栏 + 注入扫描（75 模式/9 类）+ 决策追踪 + agent 身份 KYA + 签名回执，MIT 零依赖。二者均本地/离线/OSS，作监控项。
- 🟠 **[常规·国内竞争者升温]** 中文 MCP 安全综述（数据 2026-07）列 **Tencent AI-Infra-Guard**（⭐4,301，MCP 扫描+Agent 扫描）、**AgentShield**（⭐1,023，MCP 配置/权限漏洞扫描）、Semgrep MCP（⭐681）、HexStrike AI（⭐10,515，AI Agent 渗透）等 → 国内 agent 安全赛道快速拥挤，GEO/agent-native 露出窗口期缩短，须加速。
- 🟠 **[常规·sandbox 类基础设施 GA]** **Cloudflare Sandboxes + Containers 正式全面商用（GA）**：为 AI agent 提供持久化隔离 Linux 环境（凭证注入出站代理/PTY/快照/按活跃 CPU 计费），是 agent 隔离的一等云原语，非扫描器但挤压"本地/隔离"叙事；**Goose**（Block/AAIF，Rust 原生本地 agent，52K+ stars）内置 prompt injection 检测 + sandbox mode + adversary reviewer（第二 LLM 审工具调用）→ agent harness 自带安全检测，进一步稀释独立扫描器叙事。forgevm / Open Interpreter 本周无新安全扫描功能。
- 🟢 **[我们·线上健康]** aishield.tools `GET /api/v1/health` → **4.3.0 / rules_count 235**（208 静态+8 情报+19 雷达，较 09-14 的 236 −1，属维护期微调）/ commit **eed3df14** / deployed **2026-09-20T08:38:30Z**（昨日新部署）；`.well-known/mcp/server-card.json` 4.3.0 / 235 MCP(+244 skill) / 工具名正确；Official MCP Registry `io.github.lm203688/aishield` **4.3.0 isLatest=true active**；npm `aishield-mcp-server` latest **4.3.0**（npmjs.org 出口间歇 000，由 Registry+GitHub+LobeHub+npmscan 多源交叉验证 4.3.0 仍 latest）；Glama 复测仍 live。
- 🔴 **[持续·品牌劫持未变]** LobeHub `aishield-ai/aishield` 仍 **2.1.0 / Updated 2026-08-31 / Unvalidated**，其 Packages 区**仍把全部包 + GitHub Action `aishield/audit@v1` + Claude Skill 链接到 `github.com/lm203688/aishield`（我们的仓库）**，`npx aishield-mcp` 仍 404 → 冒用劫持自 09-07 起未变（09-13 已升级为 🔴 跨项目条目，本周无新事件故不重复升级）。`aishield.ai` 首页仍 Atom 待售 $19,495；himcp.cn 仍 Not Found → 无新渠道扩张。
- ⚠️ **[持续·npm README drift]** 4.3.0 的 npm README 仍写「201 local rules / Total 215(MCP)·221(Skill)」，与真相 235(后端)/241(Skill) 不符（低估且内部自相矛盾）→ 下次发版同步（AI 不代改）。
- ℹ️ **[持续·ClawHub squat 稳定]** `ai-shield-audit`(LaurentAIA) 仍在，本周**未发现新的 aishield 同名 squat**；`mcp-audit` 命名空间 Registry count 仍为 **14**（与 08-31/09-14 同，无新 entrant）。

**行动建议（用户侧，AI 不代发/不代提）**：① 借"我们 Registry 4.3.0 vs 竞品 Registry 2.0.1（落后 2.5 年）"做 agent-native 露出，强化 aishield 干净品牌锚点；② 守夜/竞争情报已持续 09-13 升级 🔴 品牌劫持条目，待 LobeHub 提申诉；③ 防御性抢注 npm `aishield-mcp`（仍 404 空闲）；④ 差异化坚定上移「x402 机器可结算认证 + attestation 持续鉴证」，且 attestation 须做出"机器可结算"差异（meok 等已低端复制"签名报告"）；⑤ 下次发版同步 npm README 到 235/241。

---

## §1 平台版图实测表（2026-08-15，2026-08-17 复核，**2026-08-22 重大纠正**，**2026-08-24 重大威胁补充**，**2026-08-31 4.3.0 上线 + drift 修复**，**2026-09-07 品牌劫持升级 + 竞品 3.1.0**，**2026-09-14 竞品 3.5.x + 企业级新入场 + 品牌面无扩张**，**2026-09-21 竞品 v4.5.10 + 新 OSS 入场 + 品牌面无扩张**）
...
| **aishield.tools 静态发现文件** | ✅ **已修复 4.3.0（2026-08-28 随发版上线）** | 已解决 | — | done | **2026-08-31 复测 `.well-known/mcp/server-card.json` → 200，`version: 4.3.0`，描述「227 MCP rules (+233 skill rules)」，工具名正确** —— 数周 stale 4.2.0/133/错工具名 drift 已随 4.3.0 发版消除（不再需 CF Pages 手动 Retry）。**2026-09-21 复测**：card 4.3.0 / 「235 MCP rules (+244 skill rules)」/ 工具名正确 ✅ |
| **aishield.tools `/api/v1` 后端** | ✅ **已修复 4.3.0/228（2026-08-28）** | 已解决 | — | done | **2026-08-31 复测 `GET /api/v1/health` → 200 `{"version":"4.3.0","rules_count":228,"rules_breakdown":{static:204,generated:9,radar:15,total:228},"agent_first":true}`** —— 此前 stale 4.2/133 已随 4.3.0 发版消除；8 工具名一致；`.well-known/agent-card.json` 同为 4.3.0。**2026-09-21 复测**：`GET /api/v1/health` → 200 `{"version":"4.3.0","rules_count":235,"rules_breakdown":{static:208,generated:8,radar:19,total:235},"commit":"eed3df14","deployed_at":"2026-09-20T08:38:30Z"}` ✅（昨日新部署，较 09-14 的 236 −1 属维护微调） |
...
| **npm README 规则数 drift** | ⚠️ 公开面低估（4.3.0 仍存） | — | 发版同步 | LOW | **2026-08-31 复测**：npm 已发布 README 写「**201 local rules / Total 215(MCP)·221(Skill)**」，与 `get_rule_count()` 真相 **227 MCP / 233 Skill**（基线）仍不符（低估 ~12/12，且内部 110+60+23+8=201 与 Total 215 自相矛盾）。4.3.0 未修正 → 建议下次发版同步 npm README 数字（AI 不代改）；**2026-09-07 复测 npm 已发布 README 仍写「201 local rules / Total 215(MCP)·221(Skill)」，与 `get_rule_count()` 真相 227 MCP / 233 Skill 不符（低估且内部 110+60+23+8=201 与 Total 215 自相矛盾），持续 drift**；**2026-09-21 复测仍同（真相已更新为后端 235 / Skill 241，README 仍 201/215/221，低估且自相矛盾持续）** |
...
| **aishield.ai 渠道扩张** | ⚠️ 仍待售 / 无扩张 | 监控 | — | MED | **2026-08-31 复测**：`aishield.ai` 仍为 Atom「Premium Domain For Sale」（curl 直连现命中 Cloudflare 挑战页，WebFetch 确认内容未变）；LobeHub `aishield-ai/aishield` 仍 **2.1.0** 标「Local Service / 本地」、`npx aishield-mcp` 指令（包仍 404）；**himcp.cn 仍 Not Found** → 本周无新平台扩张，威胁维持「待售+本地叙事劫持」组合。**2026-09-21 复测**：`aishield.ai` 仍 Atom 待售 $19,495（WebFetch 确认）；LobeHub `aishield-ai/aishield` 仍 2.1.0/Updated 2026-08-31/Unvalidated、Packages 仍链 `github.com/lm203688/aishield`；himcp.cn 仍 Not Found → 连续第 4 周无新渠道扩张、无新同名 squat |
...
- 🔴 **2026-09-21 跟踪**：`aishield.ai` 首页**仍为 Atom「Premium Domain For Sale」（$19,495）**（WebFetch 实测确认内容未变）；LobeHub `aishield-ai/aishield` **仍 2.1.0 / Updated 2026-08-31 / Unvalidated**，Packages 区**仍把所有包 + GitHub Action `aishield/audit@v1` + Claude Skill 链接到 `github.com/lm203688/aishield`（我们的仓库）**——冒用劫持自 09-07 起连续第 3 周未变；`npx aishield-mcp` 仍 404。**himcp.cn 仍 Not Found**（复测确认）→ 本周无新渠道扩张、无新同名 squat，域名待售+本地叙事劫持组合持续。净判断：品牌劫持为"旧伤未愈"，非本周新事件，不重复升级跨项目条目；但 LobeHub 申诉（09-13 🔴）仍待用户执行。
...
#### 2026-09-21 竞品更新（直接竞品跨 v4.0 → v4.5.10 重大跃迁 + 新 OSS 入场 + sandbox 基础设施 GA）

> 本周竞品密度再升，且直接竞品 sinewaveai 在两周内 3.5.2→v4.5.10（跨越 v4.0 ProofLayer / v4.3 关键修复 / v4.4–v4.5 持续发版）；但其 Official MCP Registry 条目仍卡 2.0.1（我们 4.3.0 已 current，版本差约 2.5 年）——agent 原生发现面我们反而比它"新鲜数年"。差异化须坚定上移至 **机器可结算认证(x402) + 持续鉴证 attestation**，且 attestation 须做出"机器可结算"差异（低端玩家已复制"签名报告"）。

- 🟡 **agent-security-scanner-mcp (sinewaveai) 3.5.2 → v4.5.10（重大功能跃迁）** — 09-14 记录为 3.5.2，本周 npm/mcp.so/LobeHub/GitHub 已 **v4.4.5（GitHub latest release）/ v4.5.10（npmscan 标注 last publish 2026-09-02）**；新增：① **ProofLayer 轻量分叉**（`@prooflayer/security-scanner`：81.5KB / 4s 安装 / 400+ 规则 / 零 Python 依赖 / 7 MCP 工具，MIT 免费商用）；② **SOC2-Technical(8)+GDPR-Technical(6) 合规证据采集**（evidence persistence + pass/partial/fail/not_evaluated）；③ **ClawHub 生态扫描**（自报扫描 16,532 skills、46% 有严重漏洞）+ `scan_clawhub` 批量 CLI + ClawProof OpenClaw 插件（ClawHavoc 恶意签名 27 规则/121 模式）；④ LLM 语义代码审查（意图画像，Claude CLI 免 key）；仍含 1700+ 规则 / 12 语言 AST+taint / 自动修复(120 模板) / SARIF 2.1.0。**⚠️ 但其 Official MCP Registry 仍显示 2.0.1 isLatest（滞后约 2.5 年）→ 我们在该 agent 原生发现面比它"新鲜数年"，是差异化叙事强杠杆。** 另：竞品把 `mcp-audit`/`agent-audit`/`agent-security-audit`/`mcp-security`/`mcp-scanner` 全加进 keywords，主动吞并 mcp-audit 命名空间 → 反衬「aishield」干净品牌资产价值更高。
- 🟠 **meok-mcp-injection-scan-mcp（MEOK AI Labs，新 OSS 入场）** — MCP 注入/prompt 投毒/SSRF 扫描器（覆盖 April 2026 Anthropic MCP RCE 类、mcp-server-git CVE 链、DockerDash 注入链、OWASP LLM Top10+GenAI Red Team v1），30+ 规则 / 5 严重度，free 5/day、Starter £29/mo、Pro £79/mo、Enterprise £1,499/mo，**已带「签名安全报告 / 可验证 attestation API」**（vercel 托管 `/verify/` 端点，审计与采购团队免账号核验）→ 我们规划的 attestation 卖点正被小玩家低端复制，须加快落地并做出"机器可结算"差异（非仅"签名报告"）。PyPI 已至 1.0.14，作监控项。
- 🟠 **agent-safety-mcp（Luciferclawd，新 OSS 入场）** — 成本护栏 + 注入扫描（75 内置模式/9 类）+ 决策追踪（trace）+ agent 身份 KYA + 签名回执，MIT、零运行时依赖（纯 Python stdlib + mcp>=1.0.0）；与我们的 prompt-injection / 审计链面重叠 → 监控项。
- 🟠 **国内竞争者升温（中文生态）** — 2026-07 MCP 安全综述列 **Tencent AI-Infra-Guard**（⭐4,301，MCP 扫描+Agent 扫描）、**AgentShield**（⭐1,023，MCP 配置/权限漏洞扫描，TypeScript，CI 集成 pre-commit/GitHub Action）、Semgrep MCP（⭐681）、HexStrike AI（⭐10,515，AI Agent 渗透）。国内 agent 安全赛道快速拥挤，GEO/agent-native 露出窗口期缩短 → 须加速。
- 🟠 **sandbox 类基础设施 GA（挤压"本地/隔离"叙事）** — **Cloudflare Sandboxes + Containers 正式全面商用（GA）**：为 AI agent 提供持久化隔离 Linux 环境（安全凭证注入出站代理/PTY/快照/按活跃 CPU 计费），是 agent 隔离的一等云原语，非扫描器但削弱"本地优先"单点差异；**Goose**（Block/AAIF，Rust 原生本地 agent，52K+ stars，Apache 2.0）内置 prompt injection 检测 + sandbox mode + adversary reviewer（第二 LLM 审工具调用）+ 审计日志，可完全离线跑本地 LLM（Ollama）→ agent harness 自带安全检测，进一步稀释独立扫描器叙事。forgevm / Open Interpreter 本周无新安全扫描功能。
- ℹ️ **不变项复测（2026-09-21）**：Registry 我们 `io.github.lm203688/aishield` 仍 **4.3.0 isLatest=true active**；直接竞品 agent-security-scanner-mcp Registry 仍 **2.0.1**（npm/GitHub/LobeHub 已 v4.4.5–v4.5.10，Registry 滞后约 2.5 年）；`mcp-audit` 命名空间仍为 **count=14**（与 08-31/09-14 同，无新 entrant）；Sunglasses / NVIDIA SkillSpector / mcp-audit-scanner 等仍未上 Registry（search 0 命中）；ClawHub `ai-shield-audit` squat 仍在但无新 aishield 同名 squat；aishield.ai 仍 Atom 待售、himcp.cn 仍 Not Found、LobeHub 劫持仍 2.1.0/链接本仓库。
