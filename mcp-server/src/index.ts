...
server.tool(
  'aishield_digest',
  `AIShield紧凑信任摘要 — 几百字节拿到结论，适合每轮都要判断"能不能信"的 agent。
...
风险等级绝不比实际找到的最严重 finding 更轻（有 high 就不会报 safe），摘要里也不会出现明文凭证。`,
  {
    source_url: z.string().optional().describe('GitHub repo URL — return the current verdict as a digest'),
    configs: z.record(z.any()).optional().describe('{path: file content} MCP client config map (static analysis only)'),
    scan_result: z.record(z.any()).optional().describe('An existing scan result to compress'),
    max_findings: z.number().int().min(0).max(20).default(3).describe('How many top findings to include'),
  },
  async ({ source_url, configs, scan_result, max_findings }) => {
    try {
      const body: Record<string, unknown> = { max_findings: max_findings ?? 3 };
      if (configs) body.configs = configs;
      else if (scan_result) body.scan_result = scan_result;
      else if (source_url) body.source_url = source_url;
      else {
        return {
          content: [{ type: 'text' as const, text: 'Provide one of: configs, scan_result, source_url' }],
        };
      }
      ...
      return { content: [{ type: 'text' as const, text: `Digest failed: ${e.message}` }] };
    }
  }
);
...
async function callEco(path: string, body: Record<string, unknown>): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  try {
...
async function callEcoGet(path: string): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  try {
    const url = `${API_BASE}${path}`;
    const headers: Record<string, string> = {
      'User-Agent': `AIShield-MCP-Server/${SERVER_VERSION}`,
...
server.tool(
  'aishield_sign_agent_card',
  `对 Agent Card 做 Ed25519 或 HMAC-SHA256 签名，产出可跨系统验证的 agent 身份凭证。
...
返回 signature (base64url) + algorithm + signed_at + public_key。`,
  {
    card: z.record(z.any()).describe('Agent Card 对象 (name/version/capabilities/endpoint 等)'),
    public_key_b64: z.string().optional().describe('Ed25519 公钥 (base64url，可选，用于验证)'),
    trust_score: z.number().min(0).max(100).optional().describe('信任分 0-100'),
  },
  ({ card, public_key_b64, trust_score }) =>
    callEco('/api/v1/agent-card/sign', { card, public_key: public_key_b64, trust_score })
);

server.tool(
  'aishield_verify_agent_card',
  `验证 Agent Card 的签名完整性，检测内容被篡改。
...
关键场景：安装第三方 agent 前，用它核对 agent card 是否被中间人修改过。`,
  {
    card: z.record(z.any()).describe('带 signature 的 Agent Card 对象'),
    public_key_b64: z.string().optional().describe('Ed25519 公钥 (base64url)'),
  },
  ({ card, public_key_b64 }) =>
    callEco('/api/v1/agent-card/verify', { card, public_key: public_key_b64 })
);

server.tool(
  'aishield_export_identity',
  `从 Agent Card 导出三态身份凭证：Ed25519 签名卡片 + KYA SD-JWT + Web Bot Auth header。
...
适合 agent 首次上架或跨平台迁移时批量导出身份包。`,
  {
    card: z.record(z.any()).describe('Agent Card 对象'),
    trust_score: z.number().min(0).max(100).optional().describe('信任分'),
  },
  ({ card, trust_score }) =>
    callEco('/api/v1/agent-card/identity', { card, trust_score })
);

// ── Specialist Registry: 专业 agent 注册 ──

server.tool(
  'aishield_register_specialist',
  `注册专业 agent 到 8 域 Registry (90 天 TTL)。
...
超过 90 天需调用 renew 续期，否则进入 lapsed 状态；被撤销的 agent 永久不可再注册同域。`,
  {
    agent_id: z.string().optional().describe('agent ID (不填自动生成)'),
    domain: z.enum(['legal', 'medical', 'finance', 'education', 'engineering', 'design', 'research', 'civic']).describe('专业域'),
    agent_card: z.record(z.any()).optional().describe('关联的 Agent Card (可选)'),
    description: z.string().optional().describe('专业描述'),
    capabilities: z.array(z.string()).optional().describe('能力标签'),
    ttl_days: z.number().min(1).max(365).default(90).describe('有效期（天）'),
  },
  ({ agent_id, domain, agent_card, description, capabilities, ttl_days }) =>
    callEco('/api/v1/specialist/agents', { agent_id, domain, agent_card, description, capabilities, ttl_days })
);

// ── ERC-8004: 钱包身份绑定 ──

server.tool(
  'aishield_wrap_erc8004',
  `将以太坊钱包地址包装成 ERC-8004 agent 身份。
...
地址格式：0x + 40 位十六进制。chain_id 常见：1=主网, 5=Goerli, 8453=Base, 42161=Arbitrum。`,
  {
    wallet_address: z.string().regex(/^0x[0-9a-fA-F]{40}$/).describe('以太坊钱包地址'),
    chain_id: z.number().default(1).describe('Chain ID'),
  },
  ({ wallet_address, chain_id }) =>
    callEco('/api/v1/identity/erc8004/wrap', { wallet_address, chain_id })
);

// ── Responsibility Chain ──

server.tool(
  'aishield_chain_create',
  `创建责任链 (HMAC-SHA256 链式追加结构)。
...
返回 chain_id 后，用 chain_append 追加事件、chain_trace 追踪某个 output_ref 的完整链路。`,
  {
    chain_id: z.string().optional().describe('自定义链 ID (不填自动生成)'),
  },
  ({ chain_id }) => callEco('/api/v1/chain', { chain_id })
);

server.tool(
  'aishield_chain_append',
  `向责任链追加一条记录。

每条记录包含 actor_id / action / target / payload / ts，自动生成 HMAC 摘要链。
append 后立即返回 verify 结果——如果链被外部修改过，verify 会返回 false。`,
  {
    chain_id: z.string().describe('责任链 ID'),
    actor_id: z.string().describe('执行者 ID'),
    action: z.string().describe('动作描述'),
    target: z.record(z.any()).optional().describe('作用对象'),
    payload: z.record(z.any()).optional().describe('附加数据'),
    ts: z.string().optional().describe('时间戳 (ISO8601，不填用当前)'),
  },
  ({ chain_id, actor_id, action, target, payload, ts }) =>
    callEco(`/api/v1/chain/${encodeURIComponent(chain_id)}/append`, { actor_id, action, target, payload, ts })
);

server.tool(
  'aishield_chain_trace',
  `按 output_ref 反向追踪责任链，找出该产出物的完整责任路径。

例如：某个 AI 生成的报告由 3 个 agent 协作完成，可以用 output_ref 查到 A→B→C 的完整委托链，
并核对每一步的 HMAC 摘要是否完整。`,
  {
    chain_id: z.string().describe('责任链 ID'),
    output_ref: z.string().describe('产出物引用 ID'),
  },
  ({ chain_id, output_ref }) =>
    callEco(`/api/v1/chain/${encodeURIComponent(chain_id)}/trace`, { output_ref })
);

// ── Protocol Bridge ──

server.tool(
  'aishield_protocol_translate',
  `MCP / A2A / ACP / AP2 协议间互译。
...
支持的协议：mcp (Anthropic) / a2a (Google) / acp (Microsoft) / ap2 (Google Payments)。`,
  {
    protocol: z.enum(['mcp', 'a2a', 'acp', 'ap2']).describe('目标协议'),
    payload: z.record(z.any()).describe('源协议 payload'),
    from_protocol: z.enum(['mcp', 'a2a', 'acp', 'ap2']).optional().describe('源协议 (省略时自动识别)'),
  },
  ({ protocol, payload, from_protocol }) =>
    callEco('/api/v1/bridge/translate', { protocol, payload, from_protocol })
);

// ── Sandbox Backend ──

server.tool(
  'aishield_sandbox_backend_current',
  `获取当前主机上被选中的 sandbox backend 及其能力摘要。
...
sandbox_backend_matrix 返回全部 5 后端的完整能力矩阵。`,
  {},
  () => callEcoGet('/api/v1/sandbox/backend/current')
);

server.tool(
  'aishield_sandbox_evaluate',
  `获取当前主机 sandbox backend 评估结果 (5 后端可用性自检)。
...
与 sandbox_backend_current 的区别：本工具返回完整评估；backend_current 只返回当前选中的那一个。`,
  {},
  () => callEcoGet('/api/v1/sandbox/backend/current')
);
...
server.tool(
  'aishield_leaderboard_query',
  `查询 4 维度 leaderboard 快照 (score/domain/provider/badge)。
...
用于生态活跃度展示，也是 contributor 激励的依据之一。`,
  {
    dimension: z.enum(['score', 'domain', 'provider', 'badge']).describe('查询维度'),
    limit: z.number().min(1).max(100).default(20).describe('返回条数'),
  },
  ({ dimension, limit }) =>
    callEcoGet(`/api/v1/leaderboard/snapshot?dimension=${dimension}&limit=${limit}`)
);

// ── Contributor ──

server.tool(
  'aishield_contributor_register',
  `注册 contributor，进入 4 级激励体系。
...
贡献者等级决定其在 agent 注册审核、规则晋升、责任链裁决等环节的话语权。`,
  {
    contributor_id: z.string().optional().describe('自定义 ID'),
    name: z.string().describe('名称'),
    org: z.string().optional().describe('组织'),
    email: z.string().optional().describe('邮箱'),
    badges: z.array(z.string()).optional().describe('初始徽章'),
  },
  ({ contributor_id, name, org, email, badges }) =>
    callEco('/api/v1/contributors', { contributor_id, name, org, email, badges })
);

// ── Attestation: 信任凭证签发/验证 ──

server.tool(
  'aishield_generate_attestation',
  `生成 Trust Attestation 凭证，为已扫描的 agent 签发信任凭证。
...
生成的 attestation_id 可用于 verify 与 revoke，也可嵌入 Agent Card。`,
  {
    source_url: z.string().optional().describe('被签发 agent 的源 URL'),
    scan_report: z.record(z.any()).optional().describe('已存在的扫描报告 (省略时自动扫描)'),
    issuer: z.string().describe('签发方标识'),
    subject: z.string().describe('被签发方标识'),
  },
  ({ source_url, scan_report, issuer, subject }) =>
    callEco('/api/v1/attestations', { source_url, scan_report, issuer, subject })
);

server.tool(
  'aishield_verify_attestation',
  `验证 Trust Attestation 的完整性与有效性。
...
用于跨组织信任传递——第三方拿到 attestation 后可独立核验，无需访问签发方的扫描结果。`,
  {
    ...
    payload: z.record(z.any()).optional().describe('直接提供 payload (可选，与 ID 二选一)'),
  },
  ({ attestation_id, payload }) =>
    callEco('/api/v1/attestations/verify', { attestation_id, payload })
);

server.tool(
  'aishield_list_attestations',
  `列出所有已签发的 Trust Attestation。`,
  {
    status: z.enum(['active', 'revoked', 'expired', 'all']).default('all').describe('筛选状态'),
    limit: z.number().min(1).max(200).default(50).describe('返回条数'),
  },
  ({ status, limit }) =>
    callEco(`/api/v1/attestations?status=${status}&limit=${limit}`, {})
);

server.tool(
  'aishield_revoke_attestation',
  `撤销已签发的 Trust Attestation。

撤销后 attestation 状态变为 revoked，任何后续 verify 都会返回 invalid。
撤销事件也会写入 Evidence Bundle（如已关联）。`,
  {
    attestation_id: z.string().describe('待撤销的 attestation ID'),
    reason: z.string().describe('撤销原因'),
    revoked_by: z.string().default('system').describe('操作者'),
  },
  ({ attestation_id, reason, revoked_by }) =>
    callEco('/api/v1/attestations/revoke', { attestation_id, reason, revoked_by })
);

// ── Specialist Domains: 列出 8 个专业域 ──

server.tool(
  'aishield_list_specialist_domains',
  `列出 Specialist Registry 支持的 8 个专业域及其注册的 agent 数量。

域列表：legal / medical / finance / education / engineering / design / research / civic。
返回每域的注册数、活跃数、过期数、顶级 agent 列表。`,
  {
    include_lapsed: z.boolean().default(false).describe('是否包含已过期的 agent'),
  },
  ({ include_lapsed }) =>
    callEcoGet(`/api/v1/specialist/domains?include_lapsed=${include_lapsed}`)
);
...
server.tool(
  ...
  `创建 Evidence Bundle 实例 (HMAC-SHA256 链式审计容器)。
...
返回 run_id，后续所有 evidence_* 工具都用此 ID 定位 bundle。`,
  {
    run_id: z.string().optional().describe('自定义 run_id (不填自动生成)'),
    hmac_secret: z.string().optional().describe('HMAC 密钥 (可选，用于跨组织验证)'),
    title: z.string().optional().describe('Bundle 标题'),
  },
  ({ run_id, hmac_secret, title }) =>
    callEco('/api/v1/evidence', { run_id, hmac_secret, title })
);

server.tool(
  'aishield_evidence_add_event',
  `向 Evidence Bundle 追加一条 OCSF 事件。
...
每次追加后自动重算 HMAC 链，返回 verify 状态。`,
  {
    run_id: z.string().describe('Bundle run_id'),
    event_type: z.string().default('audit.record').describe('OCSF event type'),
    agent_id: z.string().default('unknown').describe('执行 agent'),
    action: z.string().default('record').describe('动作'),
    payload: z.record(z.any()).default({}).describe('事件 payload'),
    activity_id: z.number().default(0).describe('OCSF activity_id'),
    outcome: z.enum(['successful', 'failed', 'unknown']).default('successful').describe('结果'),
    reason: z.string().optional().describe('原因说明'),
    ttp: z.string().optional().describe('ATT&CK TTP ID (T1xxx / Gxxx / Sxxx / TAxxx)'),
  },
  ({ run_id, event_type, agent_id, action, payload, activity_id, outcome, reason, ttp }) =>
    callEco(`/api/v1/evidence/${encodeURIComponent(run_id)}/events`, { event_type, agent_id, action, payload, activity_id, outcome, reason, ttp })
);

server.tool(
  'aishield_evidence_proposal',
  `创建 Proposal-Bound 提案，等待 approver 显式批准后才允许 dispatch。
...
只有当 approver 的签名绑定到该 hash 时才允许执行。防止 approver 事后否认或修改被批准的内容。`,
  {
    run_id: z.string().describe('Bundle run_id'),
    agent_id: z.string().default('planner').describe('提案 agent'),
    action: z.string().describe('建议动作'),
    target: z.record(z.any()).default({}).describe('作用目标'),
    parameters: z.record(z.any()).default({}).describe('动作参数'),
    rationale: z.string().describe('提案理由'),
  },
  ({ run_id, agent_id, action, target, parameters, rationale }) =>
    callEco(`/api/v1/evidence/${encodeURIComponent(run_id)}/proposals`, { agent_id, action, target, parameters, rationale })
);

server.tool(
  'aishield_evidence_approve',
  `批准 Evidence Bundle 中的提案 (Proposal-Bound Approval)。

批准记录独立于提案本身，二者通过 proposal_hash 双向绑定。篡改任意一边都会导致
verify_bundle_payload 失败。scope=task 表示单次任务、scope=session 表示整个会话。`,
  {
    run_id: z.string().describe('Bundle run_id'),
    proposal_id: z.string().describe('待批准的提案 ID'),
    approver_id: z.string().default('unknown').describe('批准者 ID'),
    scope: z.enum(['task', 'session', 'global']).default('task').describe('批准作用域'),
    expires_in: z.number().min(60).max(86400).default(3600).describe('批准有效期（秒）'),
  },
  ({ run_id, proposal_id, approver_id, scope, expires_in }) =>
    callEco(`/api/v1/evidence/${encodeURIComponent(run_id)}/proposals/${encodeURIComponent(proposal_id)}/approve`, { approver_id, scope, expires_in })
);

server.tool(
  'aishield_evidence_dispatch',
  `派发已批准的 proposal，让 executor agent 执行动作。

dispatch 前必须存在有效的 approval 记录（Proposal-Bound）。执行结果被记录为 OCSF 事件，
并进入双轮独立验证流程（第一轮 exec_result，第二轮 observe 观察实际效果）。`,
  {
    run_id: z.string().describe('Bundle run_id'),
    approval_id: z.string().describe('批准记录 ID'),
    executor_id: z.string().default('unknown').describe('执行者 ID'),
    result: z.boolean().default(true).describe('执行是否成功'),
    detail: z.record(z.any()).default({}).describe('执行详情'),
  },
  ({ run_id, approval_id, executor_id, result, detail }) =>
    callEco(`/api/v1/evidence/${encodeURIComponent(run_id)}/dispatch`, { approval_id, executor_id, result, detail })
);

server.tool(
  'aishield_evidence_observe',
  `派发后的双轮独立验证：观察者独立核对 exec_result 是否真的落地生效。
...
触发 rollback。round_no 支持多次观察累积证据。`,
  {
    run_id: z.string().describe('Bundle run_id'),
    approval_id: z.string().describe('关联的 approval ID'),
    observer_id: z.string().default('unknown').describe('观察者 ID'),
    round_no: z.number().min(1).max(10).default(1).describe('观察轮次'),
    passed: z.boolean().default(true).describe('是否通过'),
    detail: z.record(z.any()).default({}).describe('观察详情'),
    confidence: z.number().min(0).max(1).default(1).describe('观察置信度'),
  },
  ({ run_id, approval_id, observer_id, round_no, passed, detail, confidence }) =>
    callEco(`/api/v1/evidence/${encodeURIComponent(run_id)}/observe`, { approval_id, observer_id, round_no, passed, detail, confidence })
);

server.tool(
  'aishield_evidence_rollback',
  `回滚已派发的动作，创建 rollback 事件并标记原 approval 失效。

对齐 CyberGuard 的 rollback 事件类型。rollback 记录本身也是 OCSF 事件，
同样进入 HMAC 链，事后可以完整还原"执行 → 观察 → 回滚"的三段证据。`,
  {
    run_id: z.string().describe('Bundle run_id'),
    approval_id: z.string().describe('待回滚的 approval ID'),
    operator_id: z.string().default('unknown').describe('操作者 ID'),
    reason: z.string().describe('回滚原因'),
  },
  ({ run_id, approval_id, operator_id, reason }) =>
    callEco(`/api/v1/evidence/${encodeURIComponent(run_id)}/rollback`, { approval_id, operator_id, reason })
);

server.tool(
  'aishield_evidence_archive',
  `归档 Evidence Bundle，生成 archive manifest 用于离线分发。
...
manifest 可直接发送到第三方 audit 端点，无需重放原始事件。`,
  {
    run_id: z.string().describe('Bundle run_id'),
    archiver_id: z.string().default('system').describe('归档者 ID'),
  },
  ({ run_id, archiver_id }) =>
    callEco(`/api/v1/evidence/${encodeURIComponent(run_id)}/archive`, { archiver_id })
);

server.tool(
  'aishield_verify_evidence_bundle',
  `离线验证 Evidence Bundle 的完整性 (跨组织传递后校验)。
...
通过 secret 一致重放完整验证。任何一条失败都会明确指出是哪一条事件、哪个字段出错。`,
  {
    bundle: z.record(z.any()).describe('Bundle payload (JSON，来自 evidence_archive 的 manifest)'),
    hmac_secret: z.string().optional().describe('HMAC 密钥'),
  },
  ({ bundle, hmac_secret }) =>
    callEco('/api/v1/evidence/verify-payload', { bundle, hmac_secret })
);

server.tool(
  'aishield_chain_migrate_to_bundle',
  `把责任链 (Responsibility Chain v1.1) 迁移为 Evidence Bundle，用于审计格式升级。
...
适合从旧责任链升级到 CyberGuard 兼容证据格式的迁移场景。`,
  {
    chain_id: z.string().describe('源责任链 ID'),
    run_id: z.string().optional().describe('目标 Bundle run_id'),
    hmac_secret: z.string().optional().describe('HMAC 密钥'),
    archiver_id: z.string().default('system').describe('归档者'),
  },
  ({ chain_id, run_id, hmac_secret, archiver_id }) =>
    callEco('/api/v1/chain/migrate', { chain_id, run_id, hmac_secret, archiver_id })
);

server.tool(
  'aishield_ship_gate_run',
  `运行 10 状态发布门禁 (ANALYZE → BLIND_TEST → CHALLENGE → GATE → PREPARE → RELEASE → OBSERVE → ARCHIVE)。
...
--emit-bundle 结束后自动创建 Evidence Bundle 记录整个生命周期。`,
  {
    target: z.string().describe('扫描目标 (仓库 URL 或本地路径)'),
    target_type: z.enum(['mcp', 'skill', 'gpt', 'prompt']).default('mcp').describe('目标类型'),
    emit_bundle: z.boolean().default(false).describe('是否生成 Evidence Bundle'),
    auto_accept_challenge: z.boolean().default(true).describe('自动接受 challenge 阶段'),
    hmac_secret: z.string().optional().describe('HMAC 密钥 (用于 bundle)'),
  },
  ({ target, target_type, emit_bundle, auto_accept_challenge, hmac_secret }) =>
    callEco('/api/v1/ship-gate/run', { target, target_type, emit_bundle, auto_accept_challenge, hmac_secret })
);

// ── Leaderboard 补充：TOP 榜 / 快照 / 贡献者排行 ──

server.tool(
  'aishield_leaderboard_top',
  `按 trust_score 排序查询 leaderboard TOP-N。
...
适合展示"本周最佳 agent"或用于推荐引擎的输入。`,
  {
    ...
    domain: z.string().optional().describe('按专业域过滤 (若指定，走 domains/{domain} 端点)'),
    min_score: z.number().min(0).max(100).default(0).describe('最低分阈值 (无 domain 时生效)'),
  },
  ({ limit, domain, min_score }) =>
    domain
...
server.tool(
  'aishield_leaderboard_snapshot',
  `获取 4 维度 leaderboard 完整快照 (score/domain/provider/badge)。

一次调用返回所有维度的完整数据，便于生态活跃度全景展示与报告生成。`,
  {
    limit: z.number().min(1).max(100).default(50).describe('每维度返回条数'),
  },
  ({ limit }) => callEcoGet(`/api/v1/leaderboard/snapshot?limit=${limit}`)
);

server.tool(
  'aishield_contributors_leaderboard',
  `查询 contributor 排行榜 (4 级激励体系)。
...
server.tool(
  'aishield_contributor_add_event',
  `给 contributor 追加一条贡献事件，累加积分。
...
不同事件有不同权重，累计到一定分即可升级 contributor 等级。`,
  {
    contributor_id: z.string().describe('贡献者 ID'),
    event_type: z.enum(['review', 'rule_patch', 'bug_report', 'attestation', 'doc', 'ship_gate']).describe('事件类型'),
    target_ref: z.string().describe('事件作用对象 (rule_id / agent_id / PR URL 等)'),
    score: z.number().min(0).max(100).default(10).describe('事件基础分'),
    metadata: z.record(z.any()).optional().describe('附加元数据'),
  },
  ({ contributor_id, event_type, target_ref, score, metadata }) =>
    callEco(`/api/v1/contributors/${encodeURIComponent(contributor_id)}/events`, { event_type, target_ref, score, metadata })
);

server.tool(
  'aishield_sandbox_backend_matrix',
  `获取 sandbox backend 完整能力矩阵 (5 后端 × 全部能力维度)。

比 sandbox_evaluate 更详细：返回每个 backend 的操作系统兼容性、资源限制支持、
seccomp / landlock / namespaces 支持、网络隔离能力、性能评级等完整字段。`,
  {},
  () => callEcoGet('/api/v1/sandbox/backend/matrix')
);

server.tool(
  'aishield_evidence_create_proposal',
  `创建 Evidence Bundle 提案 (Proposal-Bound Approval)。
...
server.tool(
  ...
  `批准 Evidence Bundle 提案 (Proposal-Bound Approval)。

等价于 evidence_approve 的规范别名。批准记录通过 proposal_hash 与提案双向绑定。`,
  {
    run_id: z.string().describe('Bundle run_id'),
    proposal_id: z.string().describe('提案 ID'),
    approver_id: z.string().default('unknown').describe('批准者'),
    scope: z.enum(['task', 'session', 'global']).default('task').describe('批准作用域'),
    expires_in: z.number().min(60).max(86400).default(3600).describe('有效期（秒）'),
  },
  ({ run_id, proposal_id, approver_id, scope, expires_in }) =>
    callEco(`/api/v1/evidence/${encodeURIComponent(run_id)}/proposals/${encodeURIComponent(proposal_id)}/approve`, { approver_id, scope, expires_in })
);
...
server.tool(
  ...
  `验证 Evidence Bundle 的 HMAC 链完整性 (bundle 视角)。
...
server.tool(
  'aishield_personal_did_create',
  `创建或查询个人 Agent 身份 (PAI DID)。
...
后续可用于签署 Capability Ticket、追溯 Agent 行动、发起 Dispute。`,
  {
    user_id: z.string().describe('自然人标识（如 email 或平台用户 id）'),
    display_name: z.string().optional().describe('显示名'),
    email: z.string().optional().describe('email'),
  },
  ({ user_id, display_name, email }) =>
    callEco('/api/v1/personal-agents/users', { user_id, display_name, email })
);

server.tool(
  'aishield_personal_instance_register',
...
server.tool(
  'aishield_personal_ticket_create',
  `为用户下的某个 Agent 实例签发 Capability Ticket（授权票据）。
...
建议所有个人 Agent 在执行敏感操作前先向用户申请一张 Ticket。`,
  {
    user_id: z.string().describe('自然人标识'),
    instance_id: z.string().describe('Agent 实例 id'),
    actions: z.array(z.string()).min(1).describe('授权的动作白名单，如 ["purchase"]'),
    scope: z.record(z.any()).optional().describe(
      '边界：{max_amount, to_domain, allowed_categories}'),
    expires_in: z.number().int().positive().max(86400).default(3600)
      .describe('TTL 秒数，最长 24h'),
    reason: z.string().optional().describe('授权原因'),
  },
  ({ user_id, instance_id, actions, scope, expires_in, reason }) =>
    callEco(`/api/v1/personal-agents/users/${encodeURIComponent(user_id)}/tickets`,
            { instance_id, actions, scope, expires_in, reason })
);

server.tool(
  'aishield_personal_budget_check',
  `对个人 Agent 的敏感操作做预算 + 风险 pre-flight 检查。
...
高危 action 类型。`,
  {
    user_id: z.string().describe('自然人标识'),
    action: z.string().describe('动作类型，如 purchase / send_email / wire_transfer'),
    amount: z.number().positive().describe('金额'),
    currency: z.string().default('CNY').describe('币种 CNY / USD'),
    target_url: z.string().optional().describe('目标 URL'),
    category: z.string().optional().describe('类别，如 groceries / utilities'),
  },
  ({ user_id, action, amount, currency, target_url, category }) =>
    callEco('/api/v1/personal-agents/budget/check',
            { user_id, action, amount, currency, target_url, category })
);

server.tool(
  'aishield_personal_budget_reserve',
  `预留一笔个人预算（下单前先冻结）。
...
aishield_personal_budget_commit 落账；下单失败调用 release。`,
  {
    user_id: z.string().describe('自然人标识'),
    order_id: z.string().describe('订单 id（用于幂等）'),
    amount: z.number().positive().describe('金额'),
    currency: z.string().default('CNY').describe('币种'),
    target: z.string().optional().describe('目标（域名或 URL）'),
    note: z.string().optional().describe('备注'),
  },
  ({ user_id, order_id, amount, currency, target, note }) =>
    callEco('/api/v1/personal-agents/budget/reserve',
            { user_id, order_id, amount, currency, target, note })
);

server.tool(
  'aishield_personal_action_record',
  `将个人 Agent 的一次行动追加到用户的 HMAC 链。
...
并对任一 action 发起 dispute。`,
  {
    ...
    payload: z.record(z.any()).optional().describe('动作负载'),
    instance_id: z.string().optional().describe('Agent 实例 id'),
    ticket_id: z.string().optional().describe('使用的授权 Ticket'),
    verdict: z.string().default('allow').describe('check 结果'),
    actor_did: z.string().optional().describe('Agent 自签名 DID'),
    note: z.string().optional().describe('备注'),
  },
  ({ user_id, action, payload, instance_id, ticket_id, verdict, actor_did, note }) =>
    callEco(`/api/v1/personal-agents/users/${encodeURIComponent(user_id)}/actions`,
            { action, payload, instance_id, ticket_id, verdict, actor_did, note })
);

server.tool(
  'aishield_personal_dispute_file',
  `用户对某条行动记录提出 dispute（"这个操作我不知情"）。

dispute 会生成可追溯的 case，包含被质疑的 action hash + 用户 reason。
可用于向 Agent 提供方或支付通道发起正式申诉，也可离线复核。`,
  {
    user_id: z.string().describe('自然人标识'),
    seq: z.number().int().positive().describe('action 链上的 seq'),
    reason: z.string().describe('争议原因'),
    description: z.string().optional().describe('详细描述'),
  },
  ({ user_id, seq, reason, description }) =>
    callEco(`/api/v1/personal-agents/users/${encodeURIComponent(user_id)}/disputes`,
            { seq, reason, description })
);

server.tool(
  'aishield_personal_connector_vet',
  `对一个 Connector manifest 做独立安全评估（Meta 官方审核之外的第二意见）。
...
明文 token、签名/过期声明、支付 quote-first、权限-描述一致性。`,
  {
    user_id: z.string().describe('发起审核的用户标识'),
    connector: z.record(z.any()).describe(
      'Connector manifest：{name, publisher, url, capabilities, scopes, install_commands, signature, expires_at, requires_payments}'),
  },
  ({ user_id, connector }) =>
